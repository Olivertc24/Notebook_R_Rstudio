...
La estructura fundamental de un gráfico en ggplot2 es simple: 
  ...
  * 1. Empiezas con la función ggplot()
  * 2. Especificas los datos y los mapeos estéticos
  * 3. Añade capas usando +.
...
labs
     ...
     x = "Mes",
     y = "Monto Total de Primas",
     caption = "Fuente de Datos: Elaboración Propia"
```
...
```r
Temas predefinidos: theme_minimal(), theme_bw(), theme_classic(), theme_light(), etc., cambian rápidamente la apariencia general del gráfico.

* Ajustes manuales con theme(): Permite un control granular sobre cada elemento del tema (texto, líneas, fondos, leyendas).
```
...
```r
 Dividen el gráfico en múltiples paneles (o "facetas") basados en una o dos variables categóricas.

   facet_wrap(~ var): Una dimensión, envuelve los paneles en filas/columnas.

   facet_grid(var_fila ~ var_columna): Dos dimensiones, crea una cuadrícula de paneles.
```
----------------------------------------------------------------------
